...
public class Utility {
	...
	public static boolean isValidPermutation(final byte[] state) {
		final int numOfTiles = state.length, dim = (int) Math.sqrt(numOfTiles);
		int inversions = 0;
	...
	public static long getPositionsAsLong(final long boardConfig, final int numOfTilesMinusOne) {
		// Creates a long that mapping tiles(i.e. array index) to position in puzzle.
		long positions = 0;
	...
	public static void displayStats(final byte[] initState) {
		...
		System.out.println();
		System.out.println("Puzzle type:          " + (numOfTiles - 1) + "-puzzle");
		...
		System.out
				.println("Paths visited:        " + INT_FORMATTER.format(Algorithm.numberVisited));
		System.out.println("States explored:      "
				+ INT_FORMATTER.format(Algorithm.numberExpanded));
		final int numOfMoves = Algorithm.shortestPath.length();
		if (numOfMoves == 1) {
			System.out.println("Shortest path:        " + numOfMoves + " move");
		} else {
			System.out.println("Shortest path:        " + numOfMoves + " moves");
		}
	...
	public static byte[] getArray(final String arrayString, final int numOfTiles)
		...
		if (i == 0) {
			...
			throw new IllegalArgumentException("Input contains only " + i + " of the " + numOfTiles
					+ " tiles.");
		} else if (i > numOfTiles) {
			throw new IllegalArgumentException("Input exceeds required " + numOfTiles + " tiles.");
		}
		...
		for (i = 0; i < numStrings.length; ++i) {
			try {
				...
				if (tile < 0 || tile >= numOfTiles) {
					throw new IllegalArgumentException("Tile " + tile + " at index " + i
							+ " is out of range.");
				}
				tiles[i] = tile;
	...
	public static void generateNextState(final char direction, final byte[] state) {
		final int p = Utility.posOfElement((byte) 0, state), dimension = (int) Math
				.sqrt(state.length);
		switch (direction) {
		case 'U':
	...
	private static byte[] getMovedTiles(final String pathStr, final byte[] initState) {
		final int pathLength = pathStr.length(), boardLength = initState.length, dimension = PuzzleConfiguration
				.getDimension();
		final byte[] movedTiles = new byte[pathLength], boardConfig = new byte[boardLength];
		int posOfSpace = -1;
